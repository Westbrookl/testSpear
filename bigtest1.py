    ...
    while i < generated:
        ...
        if is_terminal != 0:
            ...
            for j in range(len(log)):
                fo=open(log[j],"a")
                if j==0:
                    fo.write(str.format("accept ratio on time {0}:{1}",int(env.time),success/i))
                elif j==1:
                    fo.write(str.format("revenue on time {0}:{1}",int(env.time),env.total_reward/env.time))
                elif j==2:
                    fo.write(str.format("cost ratio on time {0}:{1}",int(env.time),env.total_reward/env.total_cost))
