...
def make_weighted_random_graph(n, p, weights,wmax):
    ...
    if (weights == "normal"):
        ...
        edge_weights = np.random.uniform(0, 30, len(edge_list))
        node_weights = np.random.uniform(0, 30, n)
    elif (weights == "edge_v"):
        ...
        edge_weights = np.random.uniform(0, 30, len(edge_list))
        node_weights = np.random.uniform(0, 70, n)
    elif (weights == "very_intense_edge"):
        edge_weights = np.random.uniform(0, 70, len(edge_list))
        node_weights = np.random.uniform(0, 30, n)
        ...
        edge_up=int(weights_array[1])
        edge_low=int(weights_array[2])
        node_up=int(weights_array[3])
        node_low=int(weights_array[4])
        edge_weights = np.random.uniform(edge_up, edge_low, len(edge_list))
        node_weights = np.random.uniform(node_up, node_low, n)
    emin=edge_weights.min()
    emax=edge_weights.max()
    for i in range(len(edge_list)):
        G.add_edges_from([edge_list[i]], weight=edge_weights[i]/smax)
    #print(nx.adjacency_matrix(G).todense())
    edge_weight_sum=np.sum(edge_weights)
