...
class NetworkTopology(object):
    def __init__(self, name,wmax, node_size=100, link_probability=0.5, weights=None, graph_type=0,
        ...
        self.edge_list=self.graph_topology.edges(data='weight')
        self.edge_weights_sum=0
        for line in self.edge_list:
            #print(line[2])
            self.edge_weights_sum+=line[2]
        #print(self.edge_weights_sum)
        #print(self.weight_max)
       
        self.snode_sum=np.sum(self.snode_attr)
...
class VirtualNetworkRequest(NetworkTopology):
    def __init__(self, name, start, lifetime,smax,wmax, node_size=5, link_probability=0.5, weights=None, graph_type=0,
        ...
        self.vnode_sum=np.sum(self.vnode_attr)
    ...
    def generate_vnode_attributes(self,weights):
        ...
        weights_array=weights.split("_")
        if(weights=="mini_v"):
            a=np.random.uniform(0,30,self.node_size)
        elif (weights == "edge_v"):
            a = np.random.uniform(0, 30, self.node_size)
        elif (weights == "node_v"):
            a = np.random.uniform(0, 50, self.node_size)
        elif (weights == "intense"):
            a = np.random.uniform(0, 50, self.node_size)
        elif (weights == "very_intense_node"):
            a = np.random.uniform(0, 70, self.node_size)
        elif (weights == "very_intense_edge"):
            a = np.random.uniform(0, 30, self.node_size)
        elif (weights == "very_intense_all"):
            a = np.random.uniform(0, 70, self.node_size)

        elif (weights == "very_intense"):
            a = np.random.uniform(0, 100, self.node_size)
        elif(weights_array[0]=="default"):
            node_up=int(weights_array[3])
            node_low=int(weights_array[4])
            a = np.random.uniform(node_up, node_low, self.node_size)
    ...
    def initialize_node_attributes(self, type="normal", costumize=None):
        ...
        if (type == "normal"):
            ...
            attribute_list.append({"name": "cpu_in_use", "attributes": self.vnode_attr})
            bandwidth_max = np.zeros([self.node_size])
            for (u, v) in self.graph_topology.edges():
                bandwidth_max[u - 1] += self.graph_topology.get_edge_data(u, v)["weight"]
                bandwidth_max[v - 1] += self.graph_topology.get_edge_data(u, v)["weight"]
