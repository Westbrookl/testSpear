...
def test_env(env,generated,actor,sess,log=["acpt2","rvnu2","rtio2"],type="static"):
    x=list()
    #x.append(0)
    y0=list()
    y1=list()
...
def master(network_parameter_queue, exp_queue):
    ...
    testing_list_noderank=c_env_noderank.generate_VNR_list(10)
    ...
    with tf.Session() as sess,open(LOG_FILE + '_master_0', 'w') as log_file:
        ...
        while(epoch<5):
            ...
            if(epoch<=5):
                ...
                success_noderank=test_env(master_env_noderank,10,actor,sess)
                fig1=plt.figure()
                fig1.suptitle("acceptance ratio - noderank")
                sub1=fig1.add_subplot(1,1,1)
                sub1.plot(success_noderank[1],success_noderank[2],label=success_noderank[5])
                sub1.legend(loc="upper right")

                fig2=plt.figure()
                fig2.suptitle("revenue - noderank")
                sub2=fig2.add_subplot(1,1,1)
                sub2.plot(success_noderank[1],success_noderank[3],label=success_noderank[5])

                fig3=plt.figure()
                fig3.suptitle("cost ratio - noderank")
                sub3=fig3.add_subplot(1,1,1)
                sub3.plot(success_noderank[1],success_noderank[4],label=success_noderank[5])

                fig1.savefig("acceptance_ratio_noderank_"+str(epoch)+".jpg")
                fig2.savefig("revenue_noderank_"+str(epoch)+".jpg")
                fig3.savefig("cost_ratio_noderank_"+str(epoch)+".jpg")
