...
class Environment(object):
    ...
    def generate_VNR(self,name, lifetime=1000, node_size=5, link_probability=0.5, weights="mini_v", graph_type=0,
        ...
        if(self.name=="noderank"):
            node_size=np.random.randint(low=2,high=20)
    ...
    def generate_VNR_list(self, size):
        ...
        while(len(self.testing_VNR_list)<size):
            ...
            while(index>0):
                ...
                lifetime=start+np.random.exponential(500)
                name_array=self.name.split("_")
                node_size=np.random.randint(low=2,high=10)
                if(name_array[0]=="size"):
                    node_size=np.random.randint(low=int(name_array[1]),high=int(name_array[2]))
                if(self.name=="noderank"):
                    node_size=np.random.randint(low=2,high=20)
    ...
    def fetch_new_VNR(self):
        ...
        #fo=open("ccc",'a')
        #fo.write(str.format("cpusum:{0},bandwidthsum:{1},nodesize:{2}\n",current_VNR.attribute_list[0]["attributes"],current_VNR.attribute_list[1]["attributes"],current_VNR.node_size))
    ...
    def backup_env(self):
        ...
        self.edge_reward_backup=self.edge_reward

    def restore_env(self):
        self.substrate_network = self.substrate_network_backup
        self.total_cost=self.total_cost_backup
        self.total_reward=self.total_reward_backup
        self.edge_reward=self.edge_reward_backup
        self.backup_env()
    ...
    def check_node_embedding_availability(self, action):
        ...
        if (in_use[action] < node_request):
            ...
            self.total_reward+=node_request*self.snode_max
            #self.total_cost+=node_request*self.snode_max
            #print("node:",node_request*self.snode_max)
            for i in range(len(self.substrate_network.attribute_list)):
                if (self.substrate_network.attribute_list[i]["name"] == "cpu_in_use"):
    ...
    def check_link_embedding_availability(self, shortest_path, virtual_source_node, virtual_target_node):
        ...
        self.total_cost+=cost*self.sweight_max
        self.total_reward+=reward*self.sweight_max
        self.edge_reward+=reward*self.sweight_max
        #print("reward:",reward*self.sweight_max)
        #print("cost:",cost*self.sweight_max)
        return reward/cost
